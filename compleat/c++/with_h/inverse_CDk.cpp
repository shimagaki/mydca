...
void gradient_descent(){
	...
	for(j=i+1;j<L;++j){
		...
		for(b=0;b<q;++b){
			...
			//cout << "dPsi=" << dPsi << ", eps*J=" << eps*J[i*L+j][a*q+b] << endl;
...
void outputestimator(){
    	// Coupling Constant 
	string fname = "J_model_estimator_CD_L"+to_string(L)+"_c++.dat";
	ofstream file(fname);
	for(int i=0;i<L;i++){
	for(int j=0;j<L;j++){
		for(int a=0;a<q;++a){
	...
	file.close();
    	...
    	// Magnetic Field 
	string fname_h = "h_model_estimator_CD_L"+to_string(L)+"_c++.dat";
...
void output_model_empirical(){
	/****** Coupling Constant *****/
	string fname_fij = "fij_L_CD_"+to_string(L)+"_c++.dat";
	string fname_pij = "pij_CD_L_"+to_string(L)+"_c++.dat";
	...
	for(int j=0;j<L;j++){
		for(int a=0;a<q;++a){
		for(int b=0;b<q;++b){
			file_fij << Psi[i*L+j][a*q+b] << endl; 
			file_pij << Psi_model[i*L+j][a*q+b] << endl; 
		}}}}
	file_fij.close(); file_pij.close();
	
	/****** Coupling Constant *****/
	string fname_fi = "fi_CD_L_"+to_string(L)+"_c++.dat";
	string fname_pi = "pi_CD_L_"+to_string(L)+"_c++.dat";
	ofstream file_fi(fname_fi);
	ofstream file_pi(fname_pi);
	for(int i=0;i<L;i++){
		for(int a=0;a<q;++a){
			file_fi << Psi[i][a] << endl; 
			file_pi << Psi_model[i][a] << endl; 
		}}
	file_fi.close(); file_pi.close();
