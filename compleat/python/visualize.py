...
def J_data_J_model():
    ...
    #fname_data = "J_model_estimator_L"+str(L)+"_IsingGauge.dat"
...
def visualize_estimator():
    ...
    x = np.linspace(-0.2,0.2,n_parameter)
    y = np.linspace(-0.2,0.2,n_parameter)
    plt.plot(x,y)
    plt.scatter(J_data_vec,J_model_vec)
    #plt.xlabel("true parameter")
    #plt.ylabel("model parameter")
    plt.xlabel("$f_{ij}$",size=15)
    plt.ylabel("$p_{ij}$",size=15)
    plt.grid(True)
    plt.show()
